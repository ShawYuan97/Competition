...
CFG = {
    ...
    'num_workers': 22,
    'accum_iter':
    2,  # suppoprt to do batch accumulation for backprop with effectively larger batch size
    'verbose_step': 1,  # 0 (silent),1 (progress bar), 2 (one line per epoch)
    #     'device': 'cpu'
    'device': 'cuda:0',
    'file_name': 'demo3_v1_Resnet50',
    'class_nums': 7,
    'fold_num': 5,
}
```
...
```python
#采样策略 auto
sm = SMOTE(random_state=CFG['seed'],sampling_strategy={1:3500},n_jobs=-1)
...
def get_net_loss(frozen=False):
    """定义网络结构，完成初始化
    Args:
        model(str):模型名称
    """
    finetune_net = nn.Sequential()
    #     finetune_net.in_feature = nn.Sequential(nn.Conv2d(1,3,kernel_size=3,padding=1),nn.BatchNorm2d(3),nn.ReLU())
    finetune_net.features = torchvision.models.resnet50(
        pretrained=True)  #定义特征层
...
def get_pretrain_net():
    ...
    net.fc = nn.Linear(2048, CFG['class_nums'])
    return net
...
#         break 
        ...
        train_loader, val_loader = prepare_dataloader(
            pd_res_y, trn_idx, val_idx, X_res_imgs)
        
        
        device = torch.device(CFG['device'])

        model = get_net_loss().to(device)
        scaler = GradScaler()
