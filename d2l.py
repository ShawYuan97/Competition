...
def load_data_fashion_mnist(batch_size, resize=None):
    ...
    mnist_train = torchvision.datasets.FashionMNIST(
        root="/home/yxx/桌面/dataset/deep_learning_data/", train=True, transform=trans, download=True)
    mnist_test = torchvision.datasets.FashionMNIST(
        root="/home/yxx/桌面/dataset/deep_learning_data/", train=False, transform=trans, download=True)
...
# Defined in file: ./chapter_multilayer-perceptrons/kaggle-house-price.md
def download(name, cache_dir=os.path.join('/home/sun/data/storage/yxx/DeepLearningData','data')):
    """下载一个DATA_HUB中的文件，返回它的局部文件名
    
    Args:
        name(str):文件名
        cache_dir(srt):缓存文件地址
...
#         print("安装包整个大小是：",content_size,'KB，开始下载...')
        for data in tqdm(iterable=resp.iter_content(1024),total=content_size,unit='KB',desc=name):
